  ...
  return (
    <div className="space-y-4">
      <input
        ...
        className="text-xl font-semibold w-full rounded border-2 border-header bg-card px-3 py-2"
      />
      ...
      <div className="overflow-x-auto -mx-4 px-4">
        <table className="border-collapse">
          <thead>
            <tr>
              {headers.map((h, c) => (
                <th key={c} className="border-2 border-header p-0">
                  <input
                    value={h}
                    onChange={(e) => setHeader(c, e.target.value)}
                    className="bg-card-hover font-semibold px-3 py-2.5 w-40 text-base"
                  />
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row, r) => (
              <tr key={r}>
                {row.map((cell, c) => (
                  <td key={c} className="border-2 border-header p-0">
                    <input
                      value={cell}
                      onChange={(e) => setCell(r, c, e.target.value)}
                      className="bg-card px-3 py-2.5 w-40 text-base"
                    />
                  </td>
                ))}
              </tr>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button
          type="button"
          onClick={addRow}
          className="rounded border-2 border-header bg-card px-4 min-h-11 text-sm hover:bg-card-hover w-full sm:w-auto"
        >
          + Add row
        </button>
        <button
